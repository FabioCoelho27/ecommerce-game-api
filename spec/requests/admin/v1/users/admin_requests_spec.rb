...
RSpec.describe "Admin V1 User as admin", type: :request do
	...
	describe "GET /users" do
		...
		it "returns all users" do
			get url, headers: auth_header(user)
      expect(body_json['users']).to contain_exactly *users.as_json(only: %i(name email profile id))
    end
	...
	describe "POST /users" do
		...
		context "with valid params" do
			...
			it "returns last added user" do
				post url, headers: auth_header(user), params: user_params
				expect_user = User.last.as_json(only: %i(name email profile id))
	...
	describe "PATCH /users/:id" do
		...
		context "with valid params" do
			...
			it "returns updates user" do
				patch url, headers: auth_header(user), params: user_params
				user_test.reload
				user_expect = user_test.as_json(only: %i(name email password profile id))
